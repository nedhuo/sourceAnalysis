...
public class RequestManagerRetriever implements Handler.Callback {
        ...
        if (applicationManager == null) {
          ...
          applicationManager =
              factory.build(
                  ...
                  new com.bumptech.glide4110.manager.EmptyRequestManagerTreeNode(),
  ...
  public RequestManager get(@NonNull Context context) {
    if (context == null) {
      ...
      //1. 主线程获取请求并且非Application Context
  ...
  public RequestManager get(@NonNull FragmentActivity activity) {
    if (Util.isOnBackgroundThread()) {
      //1. 如果在子线程 回去走getApplicationManager
      return get(activity.getApplicationContext());
    } else {
      //2. 主线程 去加载空白Fragment处理Glide生命周期
  ...
  public RequestManager get(@NonNull Fragment fragment) {
    com.bumptech.glide4110.util.Preconditions.checkNotNull(
        fragment.getContext(),
  ...
  public RequestManager get(@NonNull View view) {
    ...
    com.bumptech.glide4110.util.Preconditions.checkNotNull(view);
  ...
  private SupportRequestManagerFragment getSupportRequestManagerFragment(
    ...
    if (current == null) {
      //2. 再从缓存Map中获取Fragment
      current = pendingSupportRequestManagerFragments.get(fm);
      if (current == null) {
        //3. 创建空白Fragment 如果父View显示 触发onStart生命周期
        current = new SupportRequestManagerFragment();
        current.setParentFragmentHint(parentHint);
        if (isParentVisible) {
          current.getGlideLifecycle().onStart();
        }
        //4. 将fragmentManager与fragment组成键值对 保存进缓存Map
        pendingSupportRequestManagerFragments.put(fm, current);
        //5. 提交事务 添加空白Fragment
        fm.beginTransaction().add(current, FRAGMENT_TAG).commitAllowingStateLoss();
        //6. 发送Handler 从缓存Map中移除保存的键值对
  ...
  private RequestManager supportFragmentGet(
    ...
    if (requestManager == null) {
      // TODO(b/27524013): Factor out this Glide.get() call.
      Glide glide = Glide.get(context);
      //2. 工厂模式创建RequestManager
      requestManager =
          factory.build(
              glide, current.getGlideLifecycle(), current.getRequestManagerTreeNode(), context);
      //3. Fragment设置RequestManager对象 为了在Fragment生命周期变化时调用
      current.setRequestManager(requestManager);
    }
    return requestManager;
  }
  ...
  public interface RequestManagerFactory {
    ...
    RequestManager build(
        ...
        @NonNull com.bumptech.glide4110.manager.RequestManagerTreeNode requestManagerTreeNode,
        @NonNull Context context);
  }
