...
public final class LruArrayPool implements ArrayPool {
  ...
  private final com.bumptech.glide4110.load.engine.bitmap_recycle.GroupedLinkedMap<Key, Object> groupedMap = new com.bumptech.glide4110.load.engine.bitmap_recycle.GroupedLinkedMap<>();
  private final KeyPool keyPool = new KeyPool();
  private final Map<Class<?>, NavigableMap<Integer, Integer>> sortedSizes = new HashMap<>();
  private final Map<Class<?>, com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<?>> adapters = new HashMap<>();
  ...
  public synchronized <T> void put(T array) {
    ...
    com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<T> arrayAdapter = getAdapterFromType(arrayClass);
  ...
  private void evictToSize(int size) {
    while (currentSize > size) {
      ...
      com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<Object> arrayAdapter = getAdapterFromObject(evicted);
  ...
  @SuppressWarnings("unchecked")
  private <T> com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<T> getAdapterFromObject(T object) {
    return (com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<T>) getAdapterFromType(object.getClass());
  }

  @SuppressWarnings("unchecked")
  private <T> com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<T> getAdapterFromType(Class<T> arrayPoolClass) {
    com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<?> adapter = adapters.get(arrayPoolClass);
    if (adapter == null) {
    ...
    return (com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<T>) adapter;
  }
  ...
  int getCurrentSize() {
    ...
    for (Class<?> type : sortedSizes.keySet()) {
      for (Integer size : sortedSizes.get(type).keySet()) {
        com.bumptech.glide4110.load.engine.bitmap_recycle.ArrayAdapterInterface<?> adapter = getAdapterFromType(type);
